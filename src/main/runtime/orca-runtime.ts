...
export class OrcaRuntimeService {
  ...
  async updateRepo(
    ...
    if ('worktreeFolderPath' in repoUpdates || 'worktreeBasePath' in repoUpdates) {
      invalidateAuthorizedRootsCache()
    }
    this.invalidateResolvedWorktreeCache()
    this.notifier?.reposChanged()
  ...
  async createManagedWorktree(args: {
    ...
    const settings = createSettings
    const worktreePathSettings = localWorktreeLayout
      ? { workspaceDir: localWorktreeLayout.path, nestWorkspaces: localWorktreeLayout.nestWorkspaces }
    ...
    const meta = this.store.setWorktreeMeta(worktreeId, {
      ...
      orcaCreatedAt: now,
      orcaCreationSource: 'runtime',
      orcaCreationWorkspaceLayout: localWorktreeLayout
        ? {
            path: localWorktreeLayout.path,
            nestWorkspaces: localWorktreeLayout.nestWorkspaces
          }
        : getWorktreeCreationLayout(repo, settings),
      ...displayNameMeta,
      baseRef: baseBranch,
