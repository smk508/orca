...
function getRemoteCreationLayout(
  repo: Repo,
  settings: Pick<GlobalSettings, 'nestWorkspaces' | 'workspaceDir'>
): OrcaWorkspaceLayout {
  if (hasRepoWorktreeBasePath(repo)) {
    return getWorktreeCreationLayout(repo, settings)
  }
  const pathOps = isWindowsAbsolutePathLike(repo.path) ? win32 : posix
  return {
...
export async function createRemoteWorktree(
  ...
  const metaUpdates: Partial<WorktreeMeta> = {
    ...
    orcaCreationWorkspaceLayout: getRemoteCreationLayout(repo, settings),
    baseRef: baseBranch,
